...
function getCx(i) {
    return 600 + (i % 2 === 0 ? i * 4 : (i - 1) * -4)
}

function binPlotSumR(data) {
    return Plot.rectX(data, Plot.binY({
        x: "sum",
        interval: 2
    }, {
        x: "r",
...
function binPlotCount(d) {
    return Plot.rectX(d, Plot.binY({
        x: "count"
    }, {
        y: "a",
        interval: 0.001,
...
function rectPlotCount(d) {
    return Plot.plot({
        ...
        marks: [
            Plot.barX(d, Plot.groupY({x: "count"}, {y: "p"}))
        ]
    })
}

function dodgePlot(data) {
    return Plot.plot({
        insetRight: 10,
        height: 4000,
        width: 1000,
